...
StatusController.prototype.peers = function(req, res) {
  var self = this;
  this.node.services.bitcoind.getPeerInfo(function(err, peers) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp({peers: peers});
  });
};

StatusController.prototype.chainStats = function(req, res) {
  var self = this;
  this.node.services.bitcoind.getChainStats(function(err, stats) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp(stats);
