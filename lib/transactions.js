...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  // Sapling START
  if (transaction.fOverwintered && transaction.version >= 4 && transaction.version < 5) {
    transformed.valueBalance = transaction.valueBalance;
    transformed.spendDescsV4 = transaction.spendDescsV4;
    transformed.outputDescsV4 = transaction.outputDescsV4;
  ...
  if (transaction.version >= 5) {
    transformed.nConsensusBranchID = transaction.nConsensusBranchID;
    transformed.valueBalance = transaction.valueBalance;
    transformed.spendDescsV5 = transaction.spendDescsV5;
    transformed.outputDescsV5 = transaction.outputDescsV5;
    transformed.spendProofsV5 = transaction.spendProofsV5;
    transformed.spendAuthSigsV5 = transaction.spendAuthSigsV5;
    transformed.outputProofsV5 = transaction.outputProofsV5;
    if (transaction.anchorSapling) {
      transformed.anchorSapling = transaction.anchorSapling;
    }
    if (transaction.bindingSig) {
      transformed.bindingSig = transaction.bindingSig;
    }

    transformed.orchardActions = transaction.orchardActions;
    if (transaction.flagsOrchard) {
      transformed.flagsOrchard = transaction.flagsOrchard;
    }
    if (transaction.valueBalanceOrchard) {
      transformed.valueBalanceOrchard = transaction.valueBalanceOrchard;
    }
    if (transaction.proofsOrchard) {
      transformed.proofsOrchard = transaction.proofsOrchard;
    }
    if (transaction.bindingSigOrchard) {
      transformed.bindingSigOrchard = transaction.bindingSigOrchard;
    }
  }
  // Orchard END
...
TxController.prototype.transformInvTransaction = function(transaction) {
  ...
  var transformed = {
    ...
    vout: vout,
    spendDescsV4: transaction.spendDescsV4,
    outputDescsV4: transaction.outputDescsV4,
    spendDescsV5: transaction.spendDescsV5,
    outputDescsV5: transaction.outputDescsV5,
    orchardActions: transaction.orcahrdActions,
    isRBF: isRBF,
  };
