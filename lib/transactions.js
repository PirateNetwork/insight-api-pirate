...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  // Sapling START - transaction here is the RPC-JSON-derived object
  // bitcoind.js's getDetailedTransaction builds (not a bitcore-lib-pirate
  // parsed Transaction instance - see transformInvTransaction below for
  // that case), which sets spendDescs/outputDescs uniformly regardless
  // of tx version (RPC's vShieldedSpend/vShieldedOutput naming doesn't
  // distinguish v4 vs v5/v6) - previously read as spendDescsV4/V5, which
  // getDetailedTransaction never actually sets, so these fields were
  ...
  // Ironwood (v6) START
  if (transaction.version === 6 && transaction.ironwood) {
    transformed.ironwood = transaction.ironwood;
  }
  // Ironwood END
...
TxController.prototype.transformInvTransaction = function(transaction) {
  ...
  var transformed = {
    ...
    orchardActions: transaction.orchardActions,
    // v6 only (raw bitcore-lib-pirate instance, matching the sibling
    // fields above - transaction here is a real parse of the raw ZMQ tx
    // bytes, so this is already populated when present).
    ironwoodBundle: transaction.version === 6 ? transaction.ironwoodBundle : undefined,
    isRBF: isRBF,
  };
