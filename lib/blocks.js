...
function BlockController(options) {
  ...
  this.blockSummaryCache = new LRUCache({max: options.blockSummaryCacheSize || BlockController.DEFAULT_BLOCKSUMMARY_CACHE_SIZE});
  this.blockCacheConfirmations = 6;
  this.blockCache = new LRUCache({max: options.blockCacheSize || BlockController.DEFAULT_BLOCK_CACHE_SIZE});
...
BlockController.prototype.getPoolInfo = function(block) {
  ...
  var scriptPubKeyBufferRaw = block.transactions[0].outputs[0]._scriptBuffer;
 ...
 for(var k in this.poolStrings) {
    if (scriptPubKeyBufferRaw.toString("hex").match(k)) {
      return this.poolStrings[k];
    }
  }
